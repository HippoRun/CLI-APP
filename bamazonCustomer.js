    ...
    connection.query("SELECT * FROM products WHERE ?", { item_id: id }, function(err, res) {
        if(err){
            ...
            if(stock === 0 || stock<quantity) {
                ...
                var price = praseInt( res[0].price)*quantity;
...
function mysqlUpdate(stock, quantity, id, price){
    ...
    connection.query('UPDATE products SET stock_quantity = ? WHERE item_id = ?', [updateStock, id], function (error, results, fileds){
